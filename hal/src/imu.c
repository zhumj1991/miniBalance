...
#define IMU_1G_RAW            (int16_t)(1.0 / MPU6050_G_PER_LSB_2)
...
//  uartSendData((uint8_t*)gyroOut, sizeof(Axis3f));
//  uartSendData((uint8_t*)accOut, sizeof(Axis3f));
...
{
	accelBias.isBufferFilled = false;
	accelBias.isBiasValueFound = false;
}

void imuCalculateGyroBiasValue(void)
{
	gyroBias.isBufferFilled = false;
	gyroBias.isBiasValueFound = false;
}
