...
uint8_t Data_Check, Send_Status, Send_Senser, Send_RCData, Send_GpsData, Send_Offset, Send_PID1, Send_PID2, Send_PID3, Send_MotoPwm;
uint8_t data_to_send[120];

float AngleOffset_Rol, AngleOffset_Pit;
...
/////////////////////////////////////////////////////////////////////////////////////
	...
	{
		if(*(data_buf + 4) == 0x01)
			imuCalculateAccBiasValue();
		if(*(data_buf + 4) == 0x02)
			imuCalculateGyroBiasValue();
		if(*(data_buf + 4) == 0x03)
		{
			imuCalculateAccBiasValue();
			imuCalculateGyroBiasValue();
...
//			PID_PID_1.D = (float)((vs16)(*(data_buf+20)<<8) | *(data_buf+21)) / 100;
	...
	if(*(data_buf + 2) == 0x12)
	{
//			PID_PID_2.P = (float)((vs16)(*(data_buf+4)<<8) | *(data_buf+5)) /1 00;
...
void Data_Send_Status(void)
{
	...
	vs16 _temp;
	_temp = (int)(eulerRollActual * 100);
	data_to_send[_cnt++]=BYTE1(_temp);
	data_to_send[_cnt++]=BYTE0(_temp);
	
	_temp = (int)(eulerYawActual*100);
	data_to_send[_cnt++]=BYTE1(_temp);
	data_to_send[_cnt++]=BYTE0(_temp);
...
//	if(RC_Control.ARMED==0)
//		data_to_send[_cnt++]=0xA0;	//Ëø¶¨
//	else if(RC_Control.ARMED==1)
	...
	uartSendData(data_to_send, _cnt);
#else
...
void Data_Send_RCData(void)
{
	...
	Rc_D.THROTTLE = 0;
	Rc_D.YAW = eulerYawActual;
	Rc_D.ROLL = eulerRollActual;
	Rc_D.PITCH = eulerPitchActual;
	Rc_D.AUX1 = 0;
	Rc_D.AUX2 = 0;
	Rc_D.AUX3 = 0;
	Rc_D.AUX4 = 0;
